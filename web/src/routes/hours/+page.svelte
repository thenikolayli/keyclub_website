<script>
    ...
    onMount(async () => {
        ...
        try {
            ...
            lastUpdate = lastUpdate.tz("America/Los_Angeles").format("MMMM Do YYYY, h:mm:ss a")
    ...
    const animate_pbars = (pbarElement, pbarGoals, pbarHours, pbarHoursElement, setPbarStart, setPbarEnd, setAnim, currentGoal = 1) => {
        ...
        let scale = pbarHours > pbarGoals[currentGoal] ? 1 : (pbarHours - pbarGoals[currentGoal - 1]) / (pbarGoals[currentGoal] - pbarGoals[currentGoal - 1])
        ...
        gsap.to(pbarElement, {
            ...
            onComplete: () => {
                if (pbarHours > pbarGoals[currentGoal]) {
                    animate_pbars(pbarElement, pbarGoals, pbarHours, pbarHoursElement, setPbarStart, setPbarEnd, setAnim, currentGoal + 1)
                }
            },
        })
    ...
    <div class="relative w-full lg:w-1/2 mx-auto h-screen min-h-screen flex flex-col items-center p-4 text-kcblack bg-stone-200">
        <div bind:this={wrapperElement} class="w-full py-8 text-xl mt-8">
            ...
            <div class="container relative w-full mt-8">
                ...
                <h2 class="absolute w-full z-10 text-center font-bold" bind:this={atHoursElement}>{atHours}</h2>
            </div>
            <div class="container relative w-full h-6 bg-gray-400">
                <div bind:this={atElement} class="absolute left-0 top-0 h-full w-full bg-kcyellow"></div>
            </div>
            <div class="container relative w-full mt-4">
                ...
                <h2 class="absolute w-full z-10 text-center font-bold" bind:this={yrHoursElement}>{yrHours}</h2>
            </div>
            <div class="container relative w-full h-6 bg-gray-400">
                <div bind:this={yrElement} class="absolute left-0 top-0 h-full w-full bg-kcyellow"></div>
            </div>
        ...
        <form class="w-full mt-8 p-8" onsubmit={check_hours}>
            ...
            <ResponsiveButton init_text="Check hours" clicked_text="..." on_click={check_hours} can_send={canSend} />
        </form>
